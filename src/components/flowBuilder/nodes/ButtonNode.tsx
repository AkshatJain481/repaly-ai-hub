...
const ButtonNode: React.FC<NodeProps> = ({ data, id }) => {
  ...
  const getActionTypeLabel = (actionType: string) => {
    switch (actionType) {
      ...
      case 'perform_actions': return 'Perform Actions';
      case 'condition': return 'Check Condition';
      case 'randomizer': return 'Random Selection';
      case 'smart_delay': return 'Smart Delay';
      case 'start_flow': return 'Start Another Flow';
      case 'goto_step': return 'Go to Step';
    ...
    >
      ...
      <div className="mt-2 flex flex-col gap-2">
        ...
        <div className="bg-purple-700 rounded p-2 text-xs">
          ...
          <span>{getActionTypeLabel(data?.config?.actionType || 'send_message')}</span>
        </div>
      </div>
